...
async def check_analyze_rate_limit(user_id: int, redis_client: redis.Redis) -> None:  # type: ignore[type-arg]
    ...
    """
    try:
        key = f"ml_analyze_rate:{user_id}"

        # Get current count
        count_bytes = await redis_client.get(key)
        count = int(count_bytes) if count_bytes else 0

        if count >= settings.ML_ANALYZE_RATE_LIMIT:
            logger.warning(
                "ml_analyze_rate_limit_exceeded",
                user_id=user_id,
                count=count,
        ...
        logger.warning(
            "ml_analyze_rate_limit_redis_error",
            user_id=user_id,
            exc_info=True,
        )
