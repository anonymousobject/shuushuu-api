...
async def refresh_tag_cooccurrence(
    ...
    n = (await db.execute(text("SELECT COUNT(DISTINCT image_id) FROM _cooccur_vl"))).scalar() or 0
    ...
    await _exec(
        db, "ALTER TABLE _cooccur_vlf ADD PRIMARY KEY (image_id, tag_id), ADD KEY k_tag (tag_id)"
    )
