...
from sqlalchemy import CHAR, Column, DECIMAL, DateTime, Enum, Float, ForeignKeyConstraint, Index, String, Table, Text, text
from sqlalchemy.dialects.mysql import CHAR, DECIMAL, INTEGER, LONGTEXT, MEDIUMINT, SMALLINT, TINYINT, TINYTEXT, VARCHAR
...
class Banners(Base):
    ...
    date: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime, server_default=text('current_timestamp()'))


t_donations = Table(
    'donations', Base.metadata,
...
class EvaTheme(Base):
    ...
    active_month_from: Mapped[int] = mapped_column(TINYINT(2), nullable=False, server_default=text('0'))
    active_month_to: Mapped[int] = mapped_column(TINYINT(2), nullable=False, server_default=text('0'))
    active_day_from: Mapped[int] = mapped_column(TINYINT(2), nullable=False, server_default=text('0'))
    ...
    theme_content: Mapped[Optional[str]] = mapped_column(LONGTEXT)
...
class Images(Base):
    ...
    date_added: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime, server_default=text('current_timestamp()'))
    status_user_id: Mapped[Optional[int]] = mapped_column(INTEGER(10))
    status_updated: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime)
    last_updated: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime)
    last_post: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime)
    filename: Mapped[Optional[str]] = mapped_column(String(120))
    original_filename: Mapped[Optional[str]] = mapped_column(String(120))
    total_pixels: Mapped[Optional[decimal.Decimal]] = mapped_column(DECIMAL(6, 3))
    image_source: Mapped[Optional[str]] = mapped_column(String(255))
    artist: Mapped[Optional[str]] = mapped_column(String(200))
    characters: Mapped[Optional[str]] = mapped_column(Text)
    miscmeta: Mapped[Optional[str]] = mapped_column(String(255))
    replacement_id: Mapped[Optional[int]] = mapped_column(INTEGER(10))

    replacement: Mapped[Optional['Images']] = relationship('Images', remote_side=[image_id], back_populates='replacement_reverse')
    replacement_reverse: Mapped[list['Images']] = relationship('Images', remote_side=[replacement_id], back_populates='replacement')
    status_user: Mapped[Optional['Users']] = relationship('Users', foreign_keys=[status_user_id], back_populates='images')
    user: Mapped['Users'] = relationship('Users', foreign_keys=[user_id], back_populates='images_')
    users: Mapped[list['Users']] = relationship('Users', foreign_keys='[Users.bookmark]', back_populates='images1')
    favorites_: Mapped[list['Favorites']] = relationship('Favorites', back_populates='image')
    image_ratings: Mapped[list['ImageRatings']] = relationship('ImageRatings', back_populates='image')
    image_reports: Mapped[list['ImageReports']] = relationship('ImageReports', back_populates='image')
    image_reviews: Mapped[list['ImageReviews']] = relationship('ImageReviews', back_populates='image')
...
class Users(Base):
    ...
    date_joined: Mapped[datetime.datetime] = mapped_column(DateTime, nullable=False, server_default=text('current_timestamp()'))
    active: Mapped[int] = mapped_column(TINYINT(1), nullable=False, server_default=text('0'))
    ...
    timezone: Mapped[decimal.Decimal] = mapped_column(DECIMAL(5, 2), nullable=False, server_default=text('0.00'))
    email_pm_pref: Mapped[int] = mapped_column(TINYINT(1), nullable=False, server_default=text('1'))
    spoiler_warning_pref: Mapped[int] = mapped_column(TINYINT(1), nullable=False, server_default=text('1'))
    thumb_layout: Mapped[int] = mapped_column(TINYINT(1), nullable=False, server_default=text('0'))
    sorting_pref: Mapped[str] = mapped_column(VARCHAR(100), nullable=False, server_default=text("'image_id'"))
    sorting_pref_order: Mapped[str] = mapped_column(VARCHAR(10), nullable=False, server_default=text("'DESC'"))
    images_per_page: Mapped[int] = mapped_column(INTEGER(3), nullable=False, server_default=text('10'))
    show_all_images: Mapped[int] = mapped_column(TINYINT(1), nullable=False, server_default=text('0'))
    show_all_meta: Mapped[int] = mapped_column(TINYINT(1), nullable=False, server_default=text('0'))
    show_all_posts: Mapped[int] = mapped_column(TINYINT(1), nullable=False, server_default=text('0'))
    ...
    date_infected: Mapped[int] = mapped_column(INTEGER(11), nullable=False, server_default=text('0'))
    forum_id: Mapped[Optional[int]] = mapped_column(MEDIUMINT(8))
    last_login: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime, server_default=text('current_timestamp()'))
    bookmark: Mapped[Optional[int]] = mapped_column(INTEGER(10))
    location: Mapped[Optional[str]] = mapped_column(String(100))
    website: Mapped[Optional[str]] = mapped_column(String(100))
    aim: Mapped[Optional[str]] = mapped_column(String(50))
    interests: Mapped[Optional[str]] = mapped_column(String(255))
    user_title: Mapped[Optional[str]] = mapped_column(String(50))
    newpassword: Mapped[Optional[str]] = mapped_column(String(40))
    newsalt: Mapped[Optional[str]] = mapped_column(CHAR(16))
    infected: Mapped[Optional[int]] = mapped_column(TINYINT(1), server_default=text('0'))
    last_login_new: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime)

    images: Mapped[list['Images']] = relationship('Images', foreign_keys='[Images.status_user_id]', back_populates='status_user')
    images_: Mapped[list['Images']] = relationship('Images', foreign_keys='[Images.user_id]', back_populates='user')
    images1: Mapped[Optional['Images']] = relationship('Images', foreign_keys=[bookmark], back_populates='users')
    bans: Mapped[list['Bans']] = relationship('Bans', foreign_keys='[Bans.banned_by]', back_populates='users')
    bans_: Mapped[list['Bans']] = relationship('Bans', foreign_keys='[Bans.user_id]', back_populates='user')
    favorites_: Mapped[list['Favorites']] = relationship('Favorites', back_populates='user')
    image_ratings: Mapped[list['ImageRatings']] = relationship('ImageRatings', back_populates='user')
    image_reports: Mapped[list['ImageReports']] = relationship('ImageReports', back_populates='user')
    image_reviews: Mapped[list['ImageReviews']] = relationship('ImageReviews', back_populates='user')
    news: Mapped[list['News']] = relationship('News', back_populates='user')
    posts_: Mapped[list['Posts']] = relationship('Posts', foreign_keys='[Posts.last_updated_user_id]', back_populates='last_updated_user')
    posts1: Mapped[list['Posts']] = relationship('Posts', foreign_keys='[Posts.user_id]', back_populates='user')
    privmsgs: Mapped[list['Privmsgs']] = relationship('Privmsgs', foreign_keys='[Privmsgs.from_user_id]', back_populates='from_user')
    privmsgs_: Mapped[list['Privmsgs']] = relationship('Privmsgs', foreign_keys='[Privmsgs.to_user_id]', back_populates='to_user')
...
class Bans(Base):
    ...
    banned_by: Mapped[Optional[int]] = mapped_column(INTEGER(10))
    ip: Mapped[Optional[str]] = mapped_column(String(15))
    action: Mapped[Optional[str]] = mapped_column(Enum('None', 'One Week Ban', 'Two Week Ban', 'One Month Ban', 'Permanent Ban'))
    reason: Mapped[Optional[str]] = mapped_column(TINYTEXT)
    message: Mapped[Optional[str]] = mapped_column(String(255))
    date: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime, server_default=text('current_timestamp()'))
    expires: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime)

    users: Mapped[Optional['Users']] = relationship('Users', foreign_keys=[banned_by], back_populates='bans')
...
class Favorites(Base):
    ...
    user_id: Mapped[int] = mapped_column(INTEGER(10), primary_key=True)
    image_id: Mapped[int] = mapped_column(INTEGER(10), primary_key=True)
    fav_date: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime, server_default=text('current_timestamp()'))
...
class GroupPerms(Base):
    ...
    group_id: Mapped[int] = mapped_column(INTEGER(10), primary_key=True)
    perm_id: Mapped[int] = mapped_column(INTEGER(10), primary_key=True)
    permvalue: Mapped[Optional[int]] = mapped_column(TINYINT(1))
...
class ImageReports(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(['image_id'], ['images.image_id'], ondelete='CASCADE', onupdate='CASCADE', name='fk_image_reports_image_id'),
        ForeignKeyConstraint(['user_id'], ['users.user_id'], ondelete='CASCADE', onupdate='CASCADE', name='fk_image_reports_user_id'),
    ...
    category: Mapped[Optional[int]] = mapped_column(TINYINT(3))
...
class ImageReviews(Base):
    ...
    image_id: Mapped[Optional[int]] = mapped_column(INTEGER(10))
    user_id: Mapped[Optional[int]] = mapped_column(INTEGER(10))
    vote: Mapped[Optional[int]] = mapped_column(TINYINT(1))
...
class News(Base):
    ...
    news_text: Mapped[Optional[str]] = mapped_column(Text)
    date: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime, server_default=text('current_timestamp()'))
    edited: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime)
...
class Posts(Base):
    ...
    image_id: Mapped[Optional[int]] = mapped_column(INTEGER(10))
    last_updated: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime)
    last_updated_user_id: Mapped[Optional[int]] = mapped_column(INTEGER(10))

    image: Mapped[Optional['Images']] = relationship('Images', back_populates='posts_')
    last_updated_user: Mapped[Optional['Users']] = relationship('Users', foreign_keys=[last_updated_user_id], back_populates='posts_')
...
class Privmsgs(Base):
    ...
    from_user: Mapped['Users'] = relationship('Users', foreign_keys=[from_user_id], back_populates='privmsgs')
    to_user: Mapped['Users'] = relationship('Users', foreign_keys=[to_user_id], back_populates='privmsgs_')
...
class Quicklinks(Base):
    ...
    user_id: Mapped[Optional[int]] = mapped_column(INTEGER(10))
    link: Mapped[Optional[str]] = mapped_column(String(32))
...
class Tags(Base):
    ...
    date_added: Mapped[datetime.datetime] = mapped_column(DateTime, nullable=False, server_default=text('current_timestamp()'))
    type: Mapped[int] = mapped_column(TINYINT(1), nullable=False, server_default=text('1'))
    title: Mapped[Optional[str]] = mapped_column(String(150))
    desc: Mapped[Optional[str]] = mapped_column(String(200))
    ...
    inheritedfrom: Mapped[Optional['Tags']] = relationship('Tags', remote_side=[tag_id], foreign_keys=[inheritedfrom_id], back_populates='inheritedfrom_reverse')
    inheritedfrom_reverse: Mapped[list['Tags']] = relationship('Tags', remote_side=[inheritedfrom_id], foreign_keys=[inheritedfrom_id], back_populates='inheritedfrom')
...
class UserSessions(Base):
    ...
    user_id: Mapped[int] = mapped_column(INTEGER(10), nullable=False)
    last_used: Mapped[datetime.datetime] = mapped_column(DateTime, nullable=False, server_default=text('current_timestamp()'))
    ip: Mapped[str] = mapped_column(String(16), nullable=False, server_default=text("''"))
    last_view_date: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime, server_default=text('current_timestamp()'))
    lastpage: Mapped[Optional[str]] = mapped_column(String(200))
    last_search: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime, server_default=text('current_timestamp()'))
...
class TagHistory(Base):
    ...
    image_id: Mapped[Optional[int]] = mapped_column(INTEGER(10))
    tag_id: Mapped[Optional[int]] = mapped_column(INTEGER(10))
    user_id: Mapped[Optional[int]] = mapped_column(INTEGER(10))
    action: Mapped[Optional[str]] = mapped_column(CHAR(1))
    date: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime, server_default=text('current_timestamp()'))
...
class TagLinks(Base):
    ...
    image_id: Mapped[int] = mapped_column(INTEGER(10), primary_key=True)
    user_id: Mapped[Optional[int]] = mapped_column(INTEGER(10))
    date_linked: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime, server_default=text('current_timestamp()'))
