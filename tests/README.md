...
```bash
# Test Database Configuration
TEST_DATABASE_URL=postgresql+asyncpg://user:password@localhost:5432/shuushuu_pytest
```
`run-tests.sh` sets this automatically from the `POSTGRES_*` values in `.env`,
so it rarely needs setting by hand.
...
For local development, you can use the same Postgres server and credentials as your dev environment:
...
1. **Session Setup** (`setup_test_database` fixture):
   ...
   - Runs the alembic chain to head (truncate-only when already there)
